...
class PNM_TMDB
{
     ...
     *            string apikey
     * @param
     *            string language default is english
     *            
     */
    public function __construct ($apikey, $lang = 'en')
    {
    ...
    public function movieImages($idMovie) 
    {
        $images = $this->movieInfo($idMovie, "images", false);
        return $images;
    }
    ...
    public function movieCollection($idMovie)
    {
        
    
        $option = (empty($option)) ? "" : "/" . $option;
        $params = "collection/" . $idMovie . $option;
        $collection = $this->_call($params, "");
        
        return $collection;
    }
    
    
    public function collectionImages($idMovie)
    {
        $option = 'images';
     ...
     *
     * @param string $action
     *            API specific function name for in the URL
     * @param string $text
     *            Unencoded paramter for in the URL
     * @return string
     *
     */
    private function _call ($action, $text, $lang = "")
    {
        ...
        $url = PNM_TMDB::_API_URL_ . $action . "?api_key=" . $this->getApikey() .
                 "&language=" . $lang . "&" . $text;
        //echo "<pre>$url</pre>";
        $ch = curl_init();
